      ...
      <div className="shrink-0 px-6 pt-6 pb-4 space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold">{TERMS.skill} 中心</h1>
              <p className="text-xs text-(--color-muted-foreground)">
                管理和配置 AI 助手的扩展 {TERMS.skill}
              </p>
            </div>
          </div>
          <Badge variant="secondary" className="text-xs px-3 py-1">
            已启用 {enabledCount} 个 {TERMS.skill}
          </Badge>
        </div>
        ...
        <div className="relative">
          ...
          <Input
            placeholder={`搜索 ${TERMS.skill} 名称、描述或标签...`}
            value={search}
      ...
      <ScrollArea className="flex-1 px-6">
        ...
        {filteredSkills.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16 text-(--color-muted-foreground)">
            ...
            <p className="text-sm">没有找到匹配的 {TERMS.skill}</p>
          </div>
        )}
      </ScrollArea>
